#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>
#include <string.h>

// Write a function named my_malloc that serves as a ΓÇ£wrapperΓÇ¥ for malloc. 
// When we call my_malloc and ask it to allocate n bytes, it in
// turn calls malloc, tests to make sure that malloc doesnΓÇÖt return a null pointer, and then
// returns the pointer from malloc. Have my_malloc print an error message and terminate
// the program if malloc returns a null pointer.

void main() {

}
